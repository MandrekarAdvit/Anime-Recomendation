...
const Navbar = ({ watchlistCount }) => {
  ...
  // Updated text size to exactly 16px
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-emerald-900/20 px-6 py-5 md:px-12 flex justify-between items-center">
      ...
      <div className="hidden md:flex items-center gap-12">
        ...
        <Link to="/watchlist" className="relative group flex items-center gap-3">
          ...
          {watchlistCount > 0 && (
            <span className="bg-emerald-600 text-black text-[12px] font-black px-2.5 py-0.5 rounded-full animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]">
              {watchlistCount}
            </span>
          )}
        </Link>
      ...
      <div className="flex items-center gap-10">
        <Link 
          to="/login" 
          className="text-white/50 hover:text-white font-black uppercase text-[16px] tracking-widest transition-colors"
        >
          Login
        </Link>
        <Link 
          to="/register" 
          className="bg-emerald-600 hover:bg-emerald-500 text-black px-10 py-4 rounded-full font-black uppercase text-[16px] tracking-widest transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)] active:scale-95"
        >
          Sign Up
        </Link>
      </div>
